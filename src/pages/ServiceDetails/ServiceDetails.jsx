...
const ServiceDetails = () => {
  ...
  const handleSubmit = e => {
    ...
    fetch('https://the-tech-hub-server.vercel.app/review', {
      method: 'POST',
      headers: {
  ...
  const { data: comments } = useQuery({
    queryKey: [{ _id }],
    queryFn: () => fetch(`https://the-tech-hub-server.vercel.app/service/reviews/${_id}`)
      .then(res => res.json())
  })

  useEffect(() => {
    fetch(`https://the-tech-hub-server.vercel.app/reviews/`)
      .then(res => res.json())
      .then(data => setReviewsAll(data))
  }, [])
        ...
        <div>
          {
            comments?.map(comment => <div id="task-comments" className="pt-4">
              <div className="bg-white rounded-lg p-3  flex flex-col justify-center items-center md:items-start shadow-lg mb-4">
                ...
                <p style={{ width: "90%" }} className="text-gray-600 text-lg text-center md:text-left ">{comment?.time} </p>
              </div>
            </div>)
          }
